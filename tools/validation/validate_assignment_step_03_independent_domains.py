...
def main() -> int:
    ...
    # Step 3 owns the domain description, including the default finite
    # discretization that Step 5 may use when the caller does not override it.
    # These are the established two-stage regression resolutions: 25 width
    # samples and 10 output-voltage samples. Point-set domains remain unchanged.
    for domain in domains.values():
        if domain.get("candidate_values"):
            continue
        kind = str(domain.get("kind", "")).lower()
        if kind == "total_width":
            domain.setdefault("sample_count", 25)
        elif kind == "node_voltage":
            domain.setdefault("sample_count", 10)
