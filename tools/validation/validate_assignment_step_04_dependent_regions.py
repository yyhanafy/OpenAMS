...
def main() -> int:
    ...
    if a.mode=='generic':
      supported_solver_types = {
        'generic_dependency_graph',
        'matched_input_network',
        'correlated_output_pair',
      }

      checks['step5_compatible_semantics'] = (
        artifact.get('resolution_semantics')
        == 'metadata_declared_generic_region_primitives'
      )

      checks['all_groups_step5_compatible'] = all(
        g.get('solver_type', g.get('solver'))
        in supported_solver_types
        for g in artifact['groups']
      )

      checks['generic_solver_field_used'] = all(
        'solver_type' in g and 'solver' not in g
        for g in artifact['groups']
      )
    else:
