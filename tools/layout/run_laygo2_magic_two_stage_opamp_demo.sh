#!/usr/bin/env bash
set -euo pipefail

ROOT="${OPENAMS_ROOT:-$HOME/AMS-Tutorial/openams}"
LAYGO2_WS="${LAYGO2_WS:-$HOME/AMS-Tutorial/laygo2_workspace_sky130}"
VENV="${OPENAMS_VENV:-$ROOT/.venv-openams}"
MAGIC_TECH="${MAGIC_TECH:-/usr/local/share/pdk/sky130A/libs.tech/magic/sky130A.tech}"
WORK="${WORK:-/tmp/openams_laygo2_magic_two_stage_opamp}"

rm -rf "$WORK"
mkdir -p \
  "$WORK/device_n" \
  "$WORK/device_p" \
  "$WORK/layout/openams_magic" \
  "$WORK/layout/openams_test"

source "$VENV/bin/activate"
export PYTHONPATH="$LAYGO2_WS/laygo2:$LAYGO2_WS${PYTHONPATH:+:$PYTHONPATH}"

echo "===== 1. GENERATE REAL SKY130 NMOS + PMOS ====="

cat > "$WORK/generate_devices.tcl" <<EOF
source /usr/local/share/pdk/sky130A/libs.tech/magic/sky130A.tcl

# ---------------- NMOS ----------------
load NFET_TOP
box values 0 0 1 1
magic::gencell sky130::sky130_fd_pr__nfet_01v8 MNDEV w 10.0 l 0.5 nf 1 m 1

set nchild ""
foreach c [cellname list allcells] {
    if {[string match "sky130_fd_pr__nfet_01v8_*" \$c]} {
        set nchild \$c
    }
}
puts "NCHILD=\$nchild"
load \$nchild
save "$WORK/device_n/\$nchild.mag"
extract all
ext2spice lvs
ext2spice -o "$WORK/device_n/NFET.spice"

# ---------------- PMOS ----------------
load PFET_TOP
box values 0 0 1 1
magic::gencell sky130::sky130_fd_pr__pfet_01v8 MPDEV w 10.0 l 0.5 nf 1 m 1

set pchild ""
foreach c [cellname list allcells] {
    if {[string match "sky130_fd_pr__pfet_01v8_*" \$c]} {
        set pchild \$c
    }
}
puts "PCHILD=\$pchild"
load \$pchild
save "$WORK/device_p/\$pchild.mag"
extract all
ext2spice lvs
ext2spice -o "$WORK/device_p/PFET.spice"

quit -noprompt
EOF

(
  cd "$WORK"
  magic -dnull -noconsole -T "$MAGIC_TECH" "$WORK/generate_devices.tcl"
)

NCHILD_MAG="$(find "$WORK/device_n" -maxdepth 1 -name 'sky130_fd_pr__nfet_01v8_*.mag' | head -1)"
PCHILD_MAG="$(find "$WORK/device_p" -maxdepth 1 -name 'sky130_fd_pr__pfet_01v8_*.mag' | head -1)"

test -n "$NCHILD_MAG"
test -n "$PCHILD_MAG"

NCHILD="$(basename "$NCHILD_MAG" .mag)"
PCHILD="$(basename "$PCHILD_MAG" .mag)"

grep -q 'w=10 l=0.5' "$WORK/device_n/NFET.spice"
grep -q 'w=10 l=0.5' "$WORK/device_p/PFET.spice"

echo "PASS: NMOS extraction contains w=10 l=0.5"
echo "PASS: PMOS extraction contains w=10 l=0.5"
echo "NCHILD=$NCHILD"
echo "PCHILD=$PCHILD"

# Laygo2 Magic exporter expects <libname>_<cellname>.mag.
for d in openams_magic openams_test; do
  cp "$NCHILD_MAG" "$WORK/layout/$d/openams_magic_${NCHILD}.mag"
  cp "$PCHILD_MAG" "$WORK/layout/$d/openams_magic_${PCHILD}.mag"
done

export OPENAMS_2STAGE_NCHILD="$NCHILD"
export OPENAMS_2STAGE_PCHILD="$PCHILD"
export OPENAMS_2STAGE_WORK="$WORK"

echo
echo "===== 2. BUILD TWO-STAGE OPAMP IN LAYGO2 ====="

cat > "$WORK/build_two_stage_opamp.py" <<'PY'
import os
import numpy as np
import laygo2
import laygo2.object
import laygo2.interface.magic

nchild = os.environ["OPENAMS_2STAGE_NCHILD"]
pchild = os.environ["OPENAMS_2STAGE_PCHILD"]
work = os.environ["OPENAMS_2STAGE_WORK"]

# Proven Magic MOS physical access rectangles.
# These use the actual Magic PCell coordinate system.
mos_pins = {
    "S": laygo2.object.Pin(
        xy=np.array([[-52, -500], [-28, -494]]),
        layer=["metal1", "pin"], netname="S"),
    "D": laygo2.object.Pin(
        xy=np.array([[28, -500], [52, -494]]),
        layer=["metal1", "pin"], netname="D"),
    "G": laygo2.object.Pin(
        xy=np.array([[-23, -540], [23, -537]]),
        layer=["metal1", "pin"], netname="G"),
}

tn = laygo2.object.template.NativeInstanceTemplate(
    libname="openams_magic",
    cellname=nchild,
    bbox=np.array([[-125, -607], [125, 607]]),
    pins=mos_pins,
)

tp = laygo2.object.template.NativeInstanceTemplate(
    libname="openams_magic",
    cellname=pchild,
    bbox=np.array([[-125, -607], [125, 607]]),
    pins=mos_pins,
)

# ------------------------------------------------------------
# Placement
#
# PM3             PM4
#
# NM1             NM2
#
#          NM5
#
# All cells are kept R0 for this first topology regression.
# ------------------------------------------------------------

m1 = tn.generate(name="M1")
m2 = tn.generate(name="M2")
m3 = tp.generate(name="M3")
m4 = tp.generate(name="M4")
m5 = tn.generate(name="M5")
m6 = tp.generate(name="M6")
m7 = tn.generate(name="M7")

m1.xy = np.array([0, 0])
m2.xy = np.array([400, 0])
m3.xy = np.array([0, 1600])
m4.xy = np.array([400, 1600])
m5.xy = np.array([200, -1600])

# Second stage is placed in a separate right-hand routing corridor.
m6.xy = np.array([900, 1600])
m7.xy = np.array([900, -1600])

dsn = laygo2.object.database.Design(
    name="two_stage_opamp",
    libname="openams_test",
)

for inst in (m1, m2, m3, m4, m5, m6, m7):
    dsn.append(inst)

# Helper.
def rect(x1, y1, x2, y2, layer="metal1"):
    dsn.append(laygo2.object.Rect(
        xy=np.array([[x1, y1], [x2, y2]]),
        layer=[layer, "drawing"],
    ))

def pin(name, x1, y1, x2, y2, layer="metal1"):
    dsn.append(laygo2.object.Pin(
        xy=np.array([[x1, y1], [x2, y2]]),
        layer=[layer, "pin"],
        netname=name,
    ))

# ============================================================
# NTAIL:
# M1.S = M2.S = M5.D
# ============================================================

# M1/M2 source access routed DOWNWARD to avoid crossing the
# M1 drain / N1 vertical spine on metal1.
rect(-52, -700, -28, -494)
rect(348, -700, 372, -494)

# Shared NTAIL bus below the input pair.
rect(-52, -720, 372, -700)

# M5 drain access:
# M5 xy=(200,-1600), D local [28,-500]..[52,-494]
# => [228,-2100]..[252,-2094]
# route upward only to the NTAIL bus.
rect(228, -2100, 252, -700)

# ============================================================
# LEFT NODE N1:
# M1.D = M3.D = M3.G = M4.G
# diode-connected PM3 + mirror gate
# ============================================================

# M1 drain lower access.
rect(28, -500, 52, -250)

# M3 drain: translation (0,1600) -> [28,1100]..[52,1106]
rect(28, 1100, 52, 1350)

# vertical N1 spine
rect(28, -250, 52, 1350)

# M3 gate: [-23,1060]..[23,1063]
rect(-23, 1050, 52, 1075)

# M4 gate: [377,1060]..[423,1063]
rect(-23, 1050, 423, 1075)

# ============================================================
# OUTPUT:
# M2.D = M4.D
# ============================================================

# M2 drain [428,-500]..[452,-494]
rect(428, -500, 452, 1350)

# M4 drain [428,1100]..[452,1106]
# already reached by the same spine.

# ============================================================
# VDD:
# M3.S = M4.S
# ============================================================

# M3 source [-52,2094]..[-28,2100]
rect(-52, 2094, -28, 2250)

# M4 source [348,2094]..[372,2100]
rect(348, 2094, 372, 2250)

rect(-52, 2230, 372, 2250)

# ============================================================
# VSS:
# M5.S
# ============================================================

# M5 source = [148,-2100]..[172,-2094]
#
# IMPORTANT:
# Do NOT run a vertical VSS strap downward here.
# The VBIAS bus passes below M5 and that old strap caused the
# VBIAS-to-VSS short seen in extraction.
#
# M5.S is connected later through the dedicated far-left VSS
# corridor together with M7.S.

# ============================================================
# VBIAS:
# M5.G
# ============================================================

# M5 gate = [177,-2140]..[223,-2137]
rect(177, -2145, 223, -2125)

# ============================================================
# INP / INN:
# M1.G / M2.G
# ============================================================

# Leave gate shapes independent, just expose pins.

# ============================================================
# SECOND STAGE
#
# N2 / first-stage output:
# M2.D = M4.D = M6.G
#
# OUT:
# M6.D = M7.D
#
# VDD:
# M6.S joins M3.S/M4.S
#
# VSS:
# M7.S joins M5.S
#
# VBIAS:
# M7.G joins M5.G
# ============================================================

# First-stage output (N2) to M6 gate.
# Existing N2 vertical spine is x=428..452.
# M6 gate after translation is x=877..923, y=1060..1063.
# Keep a 5-unit gap from the N1/M4-gate route that ends at x=423.
rect(428, 1050, 923, 1075)

# Second-stage output:
# M6.D = [928,1100]..[952,1106]
# M7 is aligned with M5 at y=-1600:
# M7.D = [928,-2100]..[952,-2094].
#
# The OUT spine is x=928..952.  VBIAS ends at x=923, leaving
# a 5-unit physical gap, so the two nets cannot touch.
rect(928, -2100, 952, 1350)

# Extend VDD bus to M6 source.
# M6.S = [848,2094]..[872,2100]
rect(848, 2094, 872, 2250)
rect(372, 2230, 872, 2250)

# ------------------------------------------------------------
# CLEAN BOTTOM-ROW ROUTING
#
# M5 and M7 are intentionally aligned on the same y coordinate.
#
# M5.S = [148,-2100]..[172,-2094]
# M7.S = [848,-2100]..[872,-2094]
#
# M5.G = [177,-2140]..[223,-2137]
# M7.G = [877,-2140]..[923,-2137]
#
# Therefore:
#   VSS   = one horizontal bus at y~-2090
#   VBIAS = one horizontal bus at y~-2140
#
# They are parallel, separated, and never cross.
# ------------------------------------------------------------

# Shared VSS source network.
#
# Critical geometry:
#   M5 source is x=148..172 at y~-2100.
#   M5 drain/tail spine is x=228..252 and goes UP.
#   M7 source is x=848..872 at y~-2100.
#   M7 drain/OUT spine is x=928..952 and goes UP.
#
# We therefore first escape each SOURCE sideways in a thin strip
# BELOW the drain/source terminal shapes (y=-2110..-2102), then
# drop VSS only in far-left/far-right corridors outside VBIAS.
#
# M5 source: short downward stub then escape LEFT.
rect(148, -2110, 172, -2094)
rect(20, -2110, 172, -2102)

# M7 source: short downward stub then escape RIGHT.
# The horizontal strip stays below the M7 drain bottom at y=-2100.
rect(848, -2110, 872, -2094)
rect(848, -2110, 1000, -2102)

# Far-left / far-right VSS drops, both outside VBIAS x-range.
rect(20, -2320, 40, -2102)
rect(980, -2320, 1000, -2102)

# Shared VSS bus far below VBIAS and below all drain spines.
rect(20, -2340, 1000, -2320)

# Shared VBIAS gate bus.
#
# M5.G = x=177..223 at y~-2140
# M7.G = x=877..923 at y~-2140
#
# One straight horizontal bus is now safe because the VSS drops
# are at x=20..40 and x=980..1000, outside this x-range.
rect(177, -2160, 923, -2120)

# ============================================================
# Top-level pins
# ============================================================

pin("INP", -23, -540, 23, -537)
pin("INN", 377, -540, 423, -537)
pin("N2", 428, 900, 452, 950)
pin("OUT", 928, 600, 952, 650)
pin("VDD", 150, 2230, 200, 2250)
pin("VSS", 300, -2340, 350, -2320)
pin("VBIAS", 300, -2160, 350, -2120)

lib = laygo2.object.database.Library(name="openams_test")
lib.append(dsn)

out_tcl = f"{work}/export_two_stage_opamp.tcl"

laygo2.interface.magic.export(
    lib,
    filename=out_tcl,
    cellname="two_stage_opamp",
    libpath=f"{work}/layout",
    scale=1,
    tech_library="sky130A",
    gds_filename=f"{work}/two_stage_opamp.gds",
)

with open(out_tcl, "a") as f:
    f.write("\nquit -noprompt\n")

for inst in (m1, m2, m3, m4, m5, m6, m7):
    print(inst.name, inst.xy.tolist())

print("PASS: Laygo2 two-stage op-amp layout constructed")
PY

(
  cd "$LAYGO2_WS"
  python3 "$WORK/build_two_stage_opamp.py"
)

echo
echo "===== 3. EXECUTE LAYGO2 -> MAGIC EXPORT ====="

(
  cd "$WORK/layout/openams_test"
  magic -dnull -noconsole -T "$MAGIC_TECH" "$WORK/export_two_stage_opamp.tcl"
)

TOPMAG="$WORK/layout/openams_test/openams_test_two_stage_opamp.mag"
test -f "$TOPMAG"

for m in M1 M2 M3 M4 M5 M6 M7; do
  grep -q " $m" "$TOPMAG"
done

echo "PASS: top-level Magic layout contains M1..M7"

echo
echo "===== 4. MAGIC EXTRACTION ====="

cat > "$WORK/extract_two_stage_opamp.tcl" <<EOF
load openams_test_two_stage_opamp
extract all
ext2spice lvs
ext2spice -o "$WORK/two_stage_opamp_extracted.spice"
quit -noprompt
EOF

(
  cd "$WORK/layout/openams_test"
  magic -dnull -noconsole -T "$MAGIC_TECH" "$WORK/extract_two_stage_opamp.tcl"
)

SP="$WORK/two_stage_opamp_extracted.spice"
test -f "$SP"

echo
echo "===== EXTRACTED SPICE ====="
cat "$SP"

echo
echo "===== 5. TOPOLOGY REGRESSION ====="

for m in M1 M2 M3 M4 M5 M6 M7; do
  grep -qE "^X${m}[[:space:]]" "$SP"
done

M1_LINE="$(grep -E '^XM1[[:space:]]' "$SP" | head -1)"
M2_LINE="$(grep -E '^XM2[[:space:]]' "$SP" | head -1)"
M3_LINE="$(grep -E '^XM3[[:space:]]' "$SP" | head -1)"
M4_LINE="$(grep -E '^XM4[[:space:]]' "$SP" | head -1)"
M5_LINE="$(grep -E '^XM5[[:space:]]' "$SP" | head -1)"
M6_LINE="$(grep -E '^XM6[[:space:]]' "$SP" | head -1)"
M7_LINE="$(grep -E '^XM7[[:space:]]' "$SP" | head -1)"

echo "M1: $M1_LINE"
echo "M2: $M2_LINE"
echo "M3: $M3_LINE"
echo "M4: $M4_LINE"
echo "M5: $M5_LINE"
echo "M6: $M6_LINE"
echo "M7: $M7_LINE"

# ------------------------------------------------------------
# Automatic topology checks.
#
# Extracted hierarchical instance ordering for these generated cells:
#   NMOS:  S D B G <subckt>
#   PMOS:  G S D   <subckt>
#
# Checks use node relationships, not Magic-generated internal names.
# ------------------------------------------------------------
read -r _ M1_S M1_D M1_B M1_G _ <<< "$M1_LINE"
read -r _ M2_S M2_D M2_B M2_G _ <<< "$M2_LINE"
read -r _ M3_G M3_S M3_D _ <<< "$M3_LINE"
read -r _ M4_G M4_S M4_D _ <<< "$M4_LINE"
read -r _ M5_S M5_D M5_B M5_G _ <<< "$M5_LINE"
read -r _ M6_G M6_S M6_D _ <<< "$M6_LINE"
read -r _ M7_S M7_D M7_B M7_G _ <<< "$M7_LINE"

test "$M1_G" = "INP"
test "$M2_G" = "INN"

# No accidental D/S shorts.
test "$M1_D" != "$M1_S"
test "$M2_D" != "$M2_S"
test "$M5_D" != "$M5_S"
test "$M7_D" != "$M7_S"

# Shared tail node.
test "$M1_S" = "$M2_S"
test "$M1_S" = "$M5_D"

# First-stage mirror/reference node N1.
test "$M1_D" = "$M3_D"
test "$M1_D" = "$M3_G"
test "$M1_D" = "$M4_G"

# First-stage output N2 drives M6 gate.
test "$M2_D" = "$M4_D"
test "$M2_D" = "$M6_G"
test "$M2_D" = "N2"

# Second-stage output.
test "$M6_D" = "OUT"
test "$M7_D" = "OUT"

# Supplies and bias.
test "$M3_S" = "VDD"
test "$M4_S" = "VDD"
test "$M6_S" = "VDD"
test "$M5_S" = "VSS"
test "$M7_S" = "VSS"
test "$M5_G" = "VBIAS"
test "$M7_G" = "VBIAS"

# Explicit regression for the failure fixed here.
test "$M5_G" != "VSS"
test "$M7_G" != "VSS"
test "$M6_D" != "VSS"
test "$M7_D" != "VSS"
test "$M6_D" != "$M5_G"

echo "PASS: VBIAS is electrically distinct from VSS"
echo "PASS: OUT is electrically distinct from VSS and VBIAS"
echo "PASS: source-escape VSS routing is isolated from VBIAS, NTAIL, and OUT"
echo "PASS: extracted two-stage op-amp topology matches intended seven-transistor connectivity"

echo
echo "===== FINAL RESULT ====="
echo "PASS: Magic generated real SKY130 NMOS/PMOS devices at W=10um L=0.5um"
echo "PASS: Laygo2 placed seven externally generated devices"
echo "PASS: Laygo2 exported the OTA interconnect to Magic"
echo "PASS: Magic extracted the hierarchical two-stage op-amp"
echo "PASS: M1/M2 drain and source nodes are distinct"
echo "PASS: M1.S=M2.S=M5.D"
echo "PASS: M1.D=M3.D=M3.G=M4.G"
echo "PASS: M2.D=M4.D=M6.G=N2"
echo "PASS: M6.D=M7.D=OUT"
echo "PASS: M3.S=M4.S=M6.S=VDD"
echo "PASS: M5.S=M7.S=VSS"
echo "PASS: M5.G=M7.G=VBIAS"
echo "NOTE: compensation capacitor Cc is intentionally excluded from this first topology regression"
echo
echo "Artifacts: $WORK"
echo "Extracted SPICE: $SP"
echo "GDS: $WORK/two_stage_opamp.gds"
